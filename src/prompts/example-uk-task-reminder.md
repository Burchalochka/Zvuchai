# Prompt Example — Ukrainian Task with Reminder, Priority & Time Range
...
Нагадай мені важливу зустріч з командою з 14 до 15:30 у четвер
```
...
| Field | Reasoning |
|---|---|
| `title` | Stripped "Нагадай мені" (reminder trigger) and "важливу" (priority adjective) |
| `date` | "у четвер" = next Thursday from Tuesday = 2026-03-06 |
| `startTime` / `endTime` | "з 14 до 15:30" resolved directly |
| `estimatedDuration` | Derived: 15:30 − 14:00 = 90 min |
| `priority` | "важливу" → `"high"` |
| `reminder.enabled` | "Нагадай мені" → `true`; no specific reminder time → `before_start`, 10 min |
| `tags` | "зустріч з командою" → `["work"]` |
